...
Saat ini terdapat 1 peubah Y dan 9 peubah X dengan 119 observasi, misal ingin dilakukan eksplorasi dan analisis terkait peubah X3
...
plot(data$X3, data$Y,
     ...
     ylab = "IPM")
```
Scatter plot menunjukkan adanya hubungan negatif antara persentase penduduk miskin dengan Indeks Pembangunan Manusia (IPM). Terlihat dari pola titik-titik yang cenderung menurun dari kiri atas ke kanan bawah. Artinya, semakin tinggi persentase penduduk miskin, maka nilai IPM cenderung semakin rendah. Sebaliknya, daerah dengan persentase penduduk miskin yang lebih rendah umumnya memiliki nilai IPM yang lebih tinggi. Meskipun terdapat variasi pada beberapa titik, secara keseluruhan terlihat adanya kecenderungan hubungan negatif yang cukup jelas antara kedua variabel tersebut.
...
```{r}
boxplot(data$Y,
        main = "Boxplot Indeks Pembangunan Manusia (IPM)",
        ylab = "IPM")
```
Boxplot menunjukkan bahwa nilai tengah (median) IPM berada di sekitar 74–75. Sebanyak 50% data berada pada kisaran sekitar 72 hingga 79, yang ditunjukkan oleh panjang kotak (IQR). Nilai IPM terendah berada di sekitar 67, sedangkan nilai tertinggi sekitar 89. Selain itu, tidak terlihat adanya outlier, karena tidak terdapat titik yang berada di luar whisker. Hal ini menunjukkan bahwa seluruh nilai IPM masih berada dalam rentang yang wajar dan tidak ada pengamatan yang sangat menyimpang dari data lainnya.
...
```{r}
boxplot(data$X3,      main = "Boxplot Persentase Penduduk Miskin",
        ylab = "Persentase")
```
Berdasarkan boxplot, median persentase penduduk miskin berada di sekitar 9%. Sebanyak 50% data berada pada kisaran sekitar 6%–11%. Terdapat satu pencilan (outlier) dengan persentase penduduk miskin sekitar 21%, yang menunjukkan adanya daerah dengan tingkat kemiskinan yang jauh lebih tinggi dibandingkan daerah lainnya.
...
dengan Y adalah Indeks Pembangunan Manusia dan X1 adalah Rata-rata lama sekolah.

Didapatkan nilai $\hat\beta_0 = 84.25520$ artinya nilai dugaan rataan indeks pembangunan manusia ketika rata-rata lama sekolah bernilai 0 tahun adalah 84.25%, akan tetapi nilai 0 tidak ada dalam selang pengamatan rata-rata lama sekolah sehingga $\hat\beta_0$ tidak dapat didefinisikan lebih lanjut

Nilai $\hat\beta_1 = -0.93772$ artinya nilai dugaan perubahan dugaan rataan Indeks Pembangunan Manusia jika Persentase Penduduk Miskin berubah satu persen adalah sebesar 0.937%

Koefisien determinasi yang dihasilkan adalah 0.4831 atau 48.31% artinya keragaman persentase indeks Pembangunan Manusia yang dapat dijelaskan oleh Rata-rata lama sekolah adalah sebesar 48.31%, sisanya yaitu 51.69% dijelaskan oleh peubah lain yang tidak dimasukkan dalam model.
